...
*** b) With that decision update the `Jaeger` Resource name in the `SMCP` link:./smcp-2.1.1-external-jaeger-production-resource.yaml[smcp-2.1.1-external-jaeger-production-resource.yaml] with the correct one
