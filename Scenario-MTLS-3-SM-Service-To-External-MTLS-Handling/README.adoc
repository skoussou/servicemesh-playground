...
* Create _client_ Service Mesh namespace and create a service mesh control plane for the client application
...
oc apply -f smcp-2.1.1-registry_only-strict-mtls.yaml -n client-tenant  (change SMCP name in smcp-2.1.1-registry_only-strict-mtls.yaml to client-tenant) <1>
cd ../scripts/certs <2>
# Create secrets <2>
