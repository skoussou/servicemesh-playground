...
| 1 | Traffic to another external service (could be another ServiceMesh not managed with OSSM Federation)| TBD

| 1a | <<option1aencrypted, *Encrypted* External Traffic directly (via `sidecar container`)>>| TBD

|    | <<option1aunencryptedpermissive, *Variation 1: Un-Encrypted* External Traffic directly (via Sidecar) with *PERMISSIVE* internal traffic>>| TBD

|    | <<option1aunencryptedstrict, *Variation 2: Un-Encrypted* External Traffic directly (via Sidecar) with *STRICT* internal traffic>>| TBD

| 1b | <<option1bencrypted, *Encrypted* External Traffic via `egress gateway container`)>>| TBD
...
+
image::./images/7-ISTIO-CONFIG-MTLS-SIDECAR.png[title="Overall Configs Needed on Client Side",1000,300]     
image::./images/7-A-GW-IN.png[title="Client Gateway Incoming Requests",1000,300]   
image::./images/7B-IN-VS.png[title="Client VirtualService Incoming Requests",1000,300]      
image::./images/7C-OUT-SE.png[title="ServiceEntry for remote host registration - Outgoing Requests",1000,300]      
image::./images/7D-OUT-DR.png[title="DestinationRule for MTLS client handling at Sidecar - Outgoing Requests",1000,300]      
image::./images/7E-OUT-VS.png[title="VirtualService for http to https routing - Outgoing Requests",1000,300]   
+
image::./images/7-client-side.png[title="ServiceMesh - Outgoing Requests",1000,400]   
+
image::./images/7-service-side.png[title="ServiceMesh -Incoming Requests",1000,400]   
...
+
The script below will perform all of the above
----    
...
** Create `ServiceEntry` to register external details to Connect to External Service with `ServiceEntry` resource definition
