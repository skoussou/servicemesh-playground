...
1. Setup link:https://github.com/skoussou/servicemesh-playground/tree/main/Scenario-0-Deploy-In-ServiceMesh#greetings-client-service[Greetings Client Service] within a mesh (`namespace: greetings-client`) but `Greetings Service` is outside (namespace: `greetings-service`)
...
** mesh security for mtls is permissive (`automtls: true` or `mtls: false`)
...
2. Change `ServiceMeshControlPlane` setup to block external services access with `REGISTRY_ONLY` 
...
* `ServiceMeshControlPlane` setup
** `proxy.networking.trafficControl.outbound.policy: REGISTRY_ONLY`
** mesh security for mtls is stric (`mtls: true`)
* `ServiceMeshMemberRoll` include `greetings-client` namespace
* Test it and watch from the KIALI UI the requests flowing from rest-greeting-client to _external service_ established via `PassthroughCluster` 
