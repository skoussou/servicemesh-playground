...
sleep 15
echo
echo '---------------------- Step 3 - Retrieve AWS LB Addresses to setup ServiceMeshPeering  ----------------------'
sleep 7
echo
echo "LOGIN CLUSTER 1 [EAST]: "
oc login --token=$OCP_1_LOGIN_TOKEN --server=$OCP_1_LOGIN_SERVER
...
echo "TODO"
echo
echo 'Getting LB address of ingress-west-mesh on EAST SIDE to be used on WEST side ServiceMeshPeeer'
AWS_LB_SM_1=$(oc get svc ingress-west-mesh -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' -n $FED_1_SMCP_NAMESPACE)
echo "[EAST OCP AWS LB] $AWS_LB_SM_1"
sleep 7
echo
echo "LOGIN CLUSTER 2 [WEST]: "
oc login --token=$OCP_2_LOGIN_TOKEN --server=$OCP_2_LOGIN_SERVER
